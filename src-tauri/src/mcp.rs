...
- Route read-only work to Codex first. Assessment, audit, triage, survey and research tasks, the ones that change no files, go to a live Codex pane; request one if none is free. If no Codex pane can take the task, or one reports a usage or rate limit, use a Claude Sonnet pane, and a Claude Opus pane only when neither is available. Name the fallback and why in the dispatch so it shows in the record. Tasks that edit files, design with the human, and final acceptance are not covered by this rule; route those as you judge best.
...
mod tests {
    ...
    fn headless_precheck_accepts_large_multiline_briefs_and_refuses_unsupported_targets() {
        ...
        spec.program = "opencode".into();
        assert!(dispatch_mode_precheck("sess-2", &long, Some(&spec)).is_ok());
        for program in ["codex", "bash"] {
    ...
    fn brain_instructions_route_read_only_work_to_codex_with_a_fallback() {
        ...
        assert!(rule.contains("Name the fallback"));
    }
