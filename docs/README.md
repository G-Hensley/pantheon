...
| [`design/`](design/) | Design arguments that outgrew a single file, one directory per subject: `conductor-task-surface/`, `context-window/`, `headless-dispatch/`. They predate `specs/` and stay here until the work they describe has a specification to sit under. |
...
| Root `BACKLOG.md`, 8 sections | The reasoning eight open issues cite as their grounding, which they link to rather than restate. Triaged 2026-09-16: 12 delivered sections moved to [`archive/2026-09-16-backlog-delivered.md`](archive/2026-09-16-backlog-delivered.md), 8 remain and each names the issue that owns it. | Those eight issues close. The file is deleted when the last section goes, not before. |
